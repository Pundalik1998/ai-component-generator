...
const Home = () => {
  ...
  const getLanguage = (framework) => {
    if (framework.startsWith("react")) return framework === "react-ts" ? "typescript" : "javascript";
    return "html";
  };
  ...
  async function getResponse(retryCount = 0) {
    ...
    try {
      ...
      let promptContent = "";
      if (frameWork.value.startsWith("react")) {
        const isTypeScript = frameWork.value === "react-ts";
        promptContent = `
You are an experienced React developer. Create a modern, responsive React component${isTypeScript ? " in TypeScript" : ""}.

Component description: ${prompt}
Framework: React${isTypeScript ? " with TypeScript" : ""}

Requirements:
- Use functional components with hooks if needed.
- Include modern styling (Tailwind CSS preferred, or styled-components).
- Make it responsive and animated.
- Return ONLY the component code in JSX${isTypeScript ? "/TSX" : ""}, wrapped in Markdown fenced code blocks.
- Do NOT include imports, explanations, or anything else.
        `;
      } else {
        promptContent = `
...
- And give the whole code in a single HTML file.
      ...
      if (!extractedCode.trim()) {
        throw new Error("No valid code was generated. Please try rephrasing your prompt.");
      }
      ...
      if (retryCount < maxRetries && (error.code === "NETWORK_ERROR" || error.message.includes("quota"))) {
        toast.info(`Retrying... (${retryCount + 1}/${maxRetries})`);
        setTimeout(() => getResponse(retryCount + 1), 2000 * (retryCount + 1)); // Exponential backoff
        return;
      }
  ...
  const downnloadFile = () => {
    ...
    if (frameWork.value.startsWith("react")) {
      fileName = frameWork.value === "react-ts" ? "GenUI-Component.tsx" : "GenUI-Component.jsx";
      mimeType = "text/javascript";
    } else {
      fileName = "GenUI-Code.html";
      mimeType = "text/html";
    }
    ...
    <>
              ...
              <div className="h-full">
                {tab === 1 ? (
                  <Editor
                    ...
                    height="100%"
                    theme="vs-dark"
                    language={getLanguage(frameWork.value)}
                  />
                ) : frameWork.value.startsWith("react") ? (
                  <div className="w-full h-full flex items-center justify-center bg-gray-100 text-gray-800">
                    <div className="text-center">
                      ...
                      <p className="text-sm">React components cannot be previewed directly here.</p>
                      <p className="text-sm">Copy the code and use it in your React project.</p>
                    </div>
                  </div>
                ) : (
                  <iframe
      ...
      {isNewTabOpen && (
        <div className="absolute inset-0 bg-white w-screen h-screen overflow-auto">
          ...
          {frameWork.value.startsWith("react") ? (
            <div className="w-full h-[calc(100vh-60px)] flex items-center justify-center bg-gray-100 text-gray-800">
              <div className="text-center">
                <p className="text-lg font-semibold mb-2">Preview Not Available</p>
                <p className="text-sm">React components cannot be previewed directly here.</p>
                <p className="text-sm">Copy the code and use it in your React project.</p>
